...
Perfect, here's everything I'd add to the PRAXIS_DESIGN.md "Captured Additions" section, organized by priority:

Adopt Soon
Argus performance director — a dedicated subsystem separate from normal reflection that watches session metrics, reviewer pass rates, prediction accuracy, and failure patterns over time, then generates specific improvement directives for the agent itself. Not just "here are your stats" but "based on your last 10 sessions, here is what you should change about how you operate." Praxis has Brier scoring and failure tracking but no component that synthesizes those into actionable self-improvement instructions.
LLM provider routing and failover chain — a config-driven provider priority list with automatic failover. Primary: Claude. Fallback 1: Codex/OpenAI. Fallback 2: local Ollama. If the primary provider fails, rate-limits, or times out, Praxis drops to the next in chain without stopping the session. Given real-world token limits this is practically critical, not just nice to have.
Speculative execution in Act — run multiple lightweight plan branches in parallel before committing to one. Pick the branch whose rehearsal outcome best matches success criteria. Praxis already has sandboxed rehearsal mode — speculative execution extends that to comparing multiple approaches before acting rather than rehearsing one fixed plan.
Active learning runtime — a scheduled subsystem separate from the main agent loop that goes out and learns: RSS feeds, GitHub notifications, documentation crawling, synthesis into cold memory. Currently Praxis memory is passive — it only captures what happens in sessions. This makes it active — the agent grows its knowledge between sessions without operator prompting.
Time-travel debugger — session snapshots at each phase boundary written to SQLite, queryable by day/session/phase. When something goes wrong on Day 60 you can reconstruct exactly what context was loaded, what decision was made, what tools were called, and what the reviewer saw. Praxis has event logs and decision receipts but no structured replay capability.
Topology-aware goal planning — when multiple goals exist, analyze their dependencies before deciding what to work on. Goal B might be blocked by Goal A. Goal C and D might be parallelizable. Currently Praxis picks goals by priority order. This makes it pick goals by dependency graph, which prevents wasted sessions on blocked work.
VS Code extension — a lightweight praxis-ops extension that surfaces current active goal, session status, last commit, and a /run trigger directly in the editor sidebar. Small surface, high daily value for a developer operator.

Future / Optional
Postmortem generator — when a session ends with reviewer failure or eval regression, automatically generate a structured postmortem: what was attempted, what failed, what the reviewer found, what should change. Stored in SQLite and surfaced in the dashboard. Axon2 had this as part of its debugger — for Praxis it's a natural extension of the existing quality system.
Cross-session failure clustering — group similar failures across sessions to detect patterns. If the same class of mistake appears in sessions 3, 7, and 12, surface that as a named failure cluster rather than three unrelated incidents. Feeds directly into Argus.
Synthetic example generation — when the learning runtime finds something valuable, generate synthetic training examples from it and store them as structured memory. Axon2's learning runtime did this — it turns passive knowledge into active capability reinforcement.
Social runtime — a lightweight scheduled subsystem for posting updates to Bluesky or other platforms on behalf of the operator. Axonix already proved this works and has value. Keep it optional and off by default.
Budget and cost tracking — track actual token usage per session, per phase, per provider. Surface monthly burn rate in the dashboard and morning brief. Flag when a session is approaching a configured spend limit. Given your current situation with token costs this one is practically useful from day one.

From GSD (get-shit-done)
Wave execution for parallel sub-agents — plans are grouped into waves based on dependencies. Within each wave, plans run in parallel. Waves run sequentially. github Praxis has parallel sub-agents via tokio::join! but no formal dependency-aware wave grouping. This is essentially topology-aware goal planning made concrete — you'd know which tasks can run simultaneously vs which must wait for others.
Seed/forward-looking idea capture — plant a seed with trigger conditions — it surfaces at the right milestone. github Praxis has a goal backlog but no concept of ideas that should surface conditionally in the future. A goal that says "do this when X is true" is more useful than one that just sits in backlog forever.
Context rot prevention as a first-class concern — GSD explicitly solves the problem of quality degrading as context fills. Each plan is small enough to execute in a fresh context window — 200k tokens purely for implementation, zero accumulated garbage. github Praxis has context budgeting and handoff notes, but GSD's approach of sizing goals to fit clean context windows and enforcing that discipline structurally is worth formalizing.
Forensics command — post-mortem investigation of failed workflow runs — diagnoses stuck loops, missing artifacts, git anomalies. github Praxis has postmortems but a dedicated diagnostic mode that can reconstruct what went wrong in a failed session is different and more powerful.
Model profile switching — quality uses Opus for planning and execution; budget uses Sonnet for planning and Haiku for verification. github Praxis routes Haiku for lightweight tasks already, but named profiles you can switch globally with one command is cleaner than per-component config.

From Ralph
PRD-driven story execution loop — Ralph picks the highest priority story where passes: false, implements it, runs quality checks, commits if checks pass, then repeats until all stories pass. github This is a tight, clean loop that maps almost perfectly onto Praxis's goal lifecycle. The passes: true/false per-story state is simpler and more durable than Praxis's current lifecycle states and worth considering as an alternative for the dev runtime specifically.
progress.txt as append-only learnings — memory persists via git history, progress.txt, and prd.json. github Dead simple, extremely durable. Between Praxis sessions the equivalent would be an append-only learnings file that each session reads and contributes to — separate from the journal, focused purely on operational discoveries.
AGENTS.md pattern discovery — after each iteration, Ralph updates AGENTS.md with discovered patterns, gotchas, and conventions so future iterations benefit. github Praxis's LEARNINGS.md is the same idea but less structured. Ralph's framing of it as "what future instances need to know about this codebase" is sharper than just journaling.
Stop condition signal — when all stories have passes: true, Ralph outputs a completion signal and the loop exits. github Praxis has goal completion but no formal "milestone done, stop looping" signal. For the dev runtime especially, a clean stop condition prevents wasted sessions on already-complete work.

Bottom line for Codex:
From GSD: wave execution, seed/trigger-based goals, forensics mode, model profiles.
From Ralph: PRD story format for dev goals, append-only learnings file, AGENTS.md-style pattern capture, formal stop condition.
Ralph is simpler than it looks and the ideas are directly portable. GSD is more complex but the wave execution and seed concepts are genuinely additive to what Praxis already has.
